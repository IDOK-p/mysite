<?php
    ...
    function selectAllFromPostsWithUsers($table1, $table2){
        global $pdo;
        $sql = "SELECT
        t1.id, t1.title, t1.img, t1.content, t1.status, t1.id_topic,
        t2.username
    ...
    //Выборка записей (posts) с автором в категорию
    function selectAllFromPostsWithUsers1($table1, $table2, $params){
        global $pdo;
        $sql = "SELECT
        t1.id, t1.title, t1.img, t1.content, t1.status, t1.id_topic,
        t2.username
    ...
    function selectTopTopicFromPostsOnIndex($table1){
        global $pdo;
        $sql = "SELECT * FROM $table1 WHERE id_topic = 40";
        $query = $pdo->prepare($sql);
        $query->execute();
        dbCheckError($query);
        return $query->fetchAll();
